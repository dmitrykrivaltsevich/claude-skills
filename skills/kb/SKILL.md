...
1. **Read the source — text extraction first, always.** For PDFs: use `/pdf` skill's `read.py` — it extracts text as markdown including math formulas. Math may look rough in Unicode (e.g. `∫₀^∞ e^(-x²) dx`) but you CAN read it — do not render pages as images just because formulas are dense. Use `--output /tmp/chapter.json` for ranges over 5 pages to avoid terminal truncation. **NEVER use `render.py` as a substitute for `read.py`** — rendering pages to images costs 10-100× more tokens per page and WILL overflow the request body on books. Reserve `render.py` ONLY for: (a) pages where `info.py` shows `has_text: false` (scanned/image-only pages), or (b) individual pages with key diagrams you need as visual assets. For Google Docs: the downloaded markdown from `/drive` is directly readable.
