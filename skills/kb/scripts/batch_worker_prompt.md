...
- READ-ONLY on %%KB_PATH%% (knowledge/, sources/, index.md, log.md, .kb/*). The base KB may already hold merged earlier waves — check-before-create: before staging a new entity/topic/citation, search the base (search.py) — if the concept already exists, link to it instead of recreating it. Recreating existing concepts wastes your tokens and the next merge. Do not depend on cross-source links within your own wave.
...
- EVERY wikilink [[x]] must resolve: target exists in base KB or you stage it in the same run, else plain text. Every date in prose MUST be a [[YYYY]]/[[YYYY-MM]]/[[YYYY-MM-DD]] wikilink — stage the timeline entry too. If you extend a live path, read it first and stage the full extended version — never a from-scratch rewrite of a file that exists in base (merge treats yours as its successor).
