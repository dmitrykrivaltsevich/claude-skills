    ...
    for match in _TOKEN_RE.finditer(selector):
        ...
        if key is not None:
            ...
            if not isinstance(current, list):
                raise KeyError("Selector index used on non-list value")
...
def _apply_where(data: Any, where: tuple[str, str] | None) -> Any:
    ...
    if not isinstance(data, list):
        raise TypeError("--where can only be used on a list result")
    key, value = where
...
@precondition(
    lambda file_path, **_: check_file_readable(str(file_path)),
    "JSON artifact is not readable",
)
@precondition(
    lambda where=None, **_: where is None or "=" in where,
    "--where must use key=value format",
)
@precondition(
    lambda limit=None, **_: limit is None or limit >= 1,
    "limit must be >= 1",
)
def query_json_artifact(
    ...
    *,
    ...
    try:
        ...
        raise ContractViolationError(f"Invalid JSON artifact: {exc}", kind="precondition") from exc
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--selector", default="", help="Dot/index selector like pages, pages[0], pages[1].markdown")
    ...
    except (ContractViolationError, KeyError, TypeError, IndexError) as exc:
        sys.exit(f"ERROR: {exc}")

    emit_json_result(result, output_path=args.output, artifact_kind="pdf-json-query")
