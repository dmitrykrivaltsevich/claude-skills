---
name: duckduckgo
description: Searches the internet via DuckDuckGo for text, image, news, and comprehensive ranked global news. Use when the user asks to search the web, find images, look up current news on a specific topic, or wants a ranked overview of the most impactful global stories right now. Scripts — search.py (text/image/news), vision.py (visual analysis), top_news.py (comprehensive ranked world news with author + summary).
allowed-tools:
  - Bash(uv run *)
  - open_browser_page
user-invocable: true
...
- **Libraries** — `ddgs` (DDG API), `httpx` + `beautifulsoup4` (metadata fetch in `top_news.py`), `python-dateutil`
