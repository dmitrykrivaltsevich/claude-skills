...
def _parse_where(where: str | None) -> tuple[str, str] | None:
    if where is None:
        return None
    if "=" not in where:
        sys.exit("ERROR: --where must use key=value format")
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--selector", default="", help="Dot/index selector like questions, facts[0], facts[1].claim")
    ...
    except (FileNotFoundError, KeyError, TypeError, IndexError, json.JSONDecodeError) as exc:
        sys.exit(f"ERROR: {exc}")

    emit_json_result(result, output_path=args.output, artifact_kind="deep-research-json-query")
