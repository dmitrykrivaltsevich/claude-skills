"""Tests for page_query.py — reopen narrow slices from markdown page artifacts."""
...
def _load_page_query_module():
    ...
    spec = importlib.util.spec_from_file_location("deep_research_page_query", script_path)
...
class TestQueryMarkdownPage:
    def test_selects_section_by_heading(self, tmp_path: Path):
        ...
        result = page_query.query_markdown_page(page_path, heading="Evidence")

        assert result["mode"] == "heading"
        assert result["heading"] == "Evidence"
        assert "Fact line 1" in result["content"]
    ...
    def test_selects_line_range(self, tmp_path: Path):
        ...
        assert result["content"] == "Fact line 1\nFact line 2\nFact line 3"
...
class TestPageQueryCli:
    def test_main_writes_output_file_and_returns_envelope(self, tmp_path: Path, capsys):
        ...
        assert envelope["artifact_kind"] == "deep-research-page-query"
