...
| [kb](skills/kb/) | LLM-curated local knowledge bases. Extracts knowledge from sources (articles, papers, books), including know-how and hidden gems, creates richly interlinked Obsidian-compatible entries, tracks citations, detects contradictions, lints style phrases with durable accepted exceptions, runs parallel batch imports (≤10 isolated workers + ordered merge) with resumable file-backed state, canned delegation prompts, scripted mechanical lint repairs, and a deterministic post-merge audit, and uses file-backed artifacts plus JSON/page-slice reopening for long-horizon bounded-context workflows. |
