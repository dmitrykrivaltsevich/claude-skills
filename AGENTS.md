...
- Recurring mechanical file repairs the agent would otherwise repeat every run (reciprocal links, date-chain stubs, timestamp normalization) MUST live in scripts with --dry-run previews, NOT in agent edit loops. Agents keep the judgment parts (prose rewrites, exception triage, semantic merges).
- Any merge/reconcile operation MUST be a fixed point under replay: re-running it over its own output MUST change zero bytes. Empty-equivalent values (null vs "" vs []) MUST compare equal, and every merge path MUST have a replay test asserting applied==0 on the second run.
